...
protected:
	...
	char name[500][SIZE];																	//data members
	char id[500][SIZE];
...
public:																						//member functions
	Topic()
	{
		...
		{
			name[i][0] = 0;																	//default constructor
			id[i][0] = 0;
		}
	}
	...
	Topic(char name1[500][SIZE], char id1[500][SIZE])
	{
		for (int i = 0; i < 500; i++)
		{
			strcpy(name[i], name1[i]);														//parameterized constructor
			strcpy(id[i], id1[i]);
		}
	}
	...
	void editTopic()
	{
		...
		for (int i = 0; i < 500; i++)
		{
			if (name[i][0] != 0)																//edit function
			{
				if (strcmp(edit, name[i]) == 0)
				{
					for (int j = 0; j < SIZE; j++)
					{
						name[i][j] = 0;
					...
					cin.getline(name[i], SIZE, '\n');

					cout << "Enter the new ID that you want to add" << endl;
					cin >> id[i];
					cin.ignore(100, '\n');

					namecheck = checkName(name[i]);
					idcheck = idCheck(id[i]);
	...
	void saveTopic()
	{
		...
		{
																									//save function
			if (name[i][0] != 0 && !top.eof())
			{
				top << id[i] << "     " << name[i] << endl;
			}
		}
...
public:																							//member functions
	...
	Tags(char newName[SIZE][SIZE])
	{
		for (int i = 0; i < 500; i++)
		{
			strcpy(name[i], newName[i]);														//parameterized constructor
		}
	}
	...
	void saveTags()
	{
		...
		for (int i = 0; i < 500; i++)
		{
			if (name[i][0] != 0 && !tags.eof())															//save function
			{
				tags << name[i] << endl;
			}
		}
	...
	void loadTags()
	{
		...
		for (int i = 0; i < 500; i++)
		{
			if (name[i][0] == 0 && !tags.eof())																//load function
			{
				tags >> name[i];
			}
		}
	...
	void getTags(char tags[500][SIZE])
	{
		for (int i = 0; i < 500; i++)
		{
			if (name[i][0] != 0)
			{
				strcpy(tags[i], name[i]);																//getter function
			}
		}
	}
};
...
public:																									//member functions
	...
	void addDoc(char catList[500][SIZE])
	{
		char check[SIZE] = { 0 };
		bool valid = true;
	...
	void editDoc(char catList[500][SIZE])
	{
		char name[SIZE] = { 0 };
		bool valid = true;
...
int main()
{
	...
	char choice, arr;																					//initialization
g:;
